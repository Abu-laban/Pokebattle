...
export function addStatus(member, statusType, addLog) {
  ...
  if (statusType === 'FRZ' && member.poke.types.includes('ICE'))  return false;
  if (statusType === 'FRZ' && member.statuses['BRN'])              return false;
...
export function applyEndTurnStatus(member, weather, addLog) {
  ...
  if (member.statuses?.['BRN']) {
    ...
    if (member.hp <= 0) { member.fainted = true; return; }
  }
